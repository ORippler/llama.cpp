...
typedef void (*mmq_write_back_t)(const float * __restrict__ sum, const int32_t * __restrict__ get_rows_to_sorted,
    const float * __restrict__ scale_y, bool scale_y_per_col, float * __restrict__ dst, const int stride, const int i_max, const int j_max);
...
static __device__ __forceinline__ void mmq_write_back_dp4a(
        ...
        const float * __restrict__ scale_y, bool scale_y_per_col, float * __restrict__ dst,
...
#pragma unroll
        for (int i0 = 0; i0 < mmq_y; i0 += warp_size) {
            ...
            const float scale = scale_y ? (scale_y_per_col ? scale_y[j] : scale_y[0]) : 1.0f;
            dst[ids_dst[j]*stride + i] = sum[(j0/nwarps) * (mmq_y/warp_size) + i0/warp_size] * scale;
        }
    }
}
...
#pragma unroll
            for (int l = 0; l < tile_C::ne; ++l) {
                ...
                const float scale = scale_y ? (scale_y_per_col ? scale_y[j] : scale_y[0]) : 1.0f;
                dst[ids_dst[j]*stride + i] = sum[(j0/tile_C::J + n)*tile_C::ne + l] * scale;
            }
        }
    }
}
...
static __device__ __forceinline__ void mul_mat_q_process_tile(
        ...
        const int * __restrict__ ids_dst, const float * __restrict__ scale_y, const bool scale_y_per_col,
...
#pragma unroll
    ...
    if (fixup) {
        write_back(sum, ids_dst, scale_y, scale_y_per_col, tmp_fixup + blockIdx.x*(mmq_x*mmq_y), mmq_y, mmq_y, mmq_x);
    } else {
        write_back(sum, ids_dst, scale_y, scale_y_per_col, dst, stride_col_dst, tile_x_max_i, tile_y_max_j);
    }
}
...
static __global__ void mul_mat_q(
        const char * __restrict__ x, const int * __restrict__ y, const float * __restrict__ scales_y, const int64_t n_scales_y,
...
#pragma unroll
        ...
        const bool scale_y_per_col = n_scales_y != 1;
        const float * scale_y = scales_y ? (scale_y_per_col ? scales_y + col_low + jt*mmq_x : scales_y) : nullptr;

        constexpr bool fixup = false;
        mul_mat_q_process_tile<type, mmq_x, need_check, fixup>
            (x, offset_x, y + offset_y, ids_dst_shared, scale_y, scale_y_per_col, dst + offset_dst, tmp_fixup, stride_row_x, ncols_y, stride_col_dst,
...
struct mmq_args {
    const char * x; ggml_type type_x; const int * y; const float * scales_y; int64_t n_scales_y; const int32_t * ids_dst; const int32_t * expert_bounds; float * dst;
...
static void launch_mul_mat_q(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    if (!args.use_stream_k) {
        if (args.nrows_x % mmq_y == 0) {
            ...
            mul_mat_q<type, mmq_x, need_check><<<block_nums_xy_tiling, block_dims, nbytes_shared, stream>>>
                (args.x, args.y, args.scales_y, args.n_scales_y, args.ids_dst, args.expert_bounds, args.dst, nullptr,
    ...
    if (args.nrows_x % mmq_y == 0) {
        ...
        mul_mat_q<type, mmq_x, need_check><<<block_nums_stream_k, block_dims, nbytes_shared, stream>>>
            (args.x, args.y, args.scales_y, args.n_scales_y, args.ids_dst, args.expert_bounds, args.dst, tmp_fixup.ptr,
             blocks_per_ne00_fd, args.nrows_x, args.ncols_dst, args.stride_row_x, args.ncols_y, args.nrows_dst,
             channel_ratio_fd, nchannels_y_fd, args.stride_channel_x, args.stride_channel_y, args.stride_channel_dst,
             sample_ratio_fd, nsamples_y_fd, args.stride_sample_x, args.stride_sample_y, args.stride_sample_dst,
             ntx_fd);
